...
import { traditionData } from "../constants/constants";
...
function DebateSection1({ title }) {
  ...
  const handleUpload = () => {
    traditionData.push({ idea: idea, date: "17/02/2025" });
    setIsCreate(false);
  };
  ...
  return (
    <div className="mt-10">
      ...
      <div className="container mx-auto bg-gray-50 p-10">
        ...
        {traditionData.map((idea, index) => (
          <MainDebate key={index} idea={idea.idea} date={idea.date} />
        ))}
      </div>
    </div>
  );
